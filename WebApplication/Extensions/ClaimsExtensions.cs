...
namespace WebApplication.Extensions
{
    public static class ClaimsExtensions
    {
        ...
        static int GetIdSystemUser(this ClaimsIdentity identity)
        {
            return int.Parse(identity.Claims?.FirstOrDefault(c => c.Type == "WebApplication.Models.UserViewModel.idSystemUser")?.Value);
        }

        public static int GetIdSystemUser(this IIdentity identity)
        {
            var claimsIdentity = identity as ClaimsIdentity;
            return claimsIdentity != null ? GetIdSystemUser(claimsIdentity) : 0;
        }
